...
<!-- Page Content -->
<div id="main">
    <div class="w3-padding-16 w3-content w3-text-grey" style="width: 100%; height: 60px">
         <div class="w3-padding-16 w3-content w3-text-grey" style="width: 800px; height: 60px;">
            <div style="background-color:white; width:100%; height:100%;  text-align: center;">
                <input id="headline" placeholder="Please wait for the data to be loaded" style="width:100%" disabled>
            </div>
        </div>
    </div>
    <div class="views w3-padding-16 w3-content w3-text-grey">
        <div class="views w3-padding-16 w3-content w3-text-grey" id="gamearea" style="background-color: deepskyblue; width: 800px; height: 450px">
            <p id="clock" style="position: absolute; padding-left: 25px; color: black"></p><br>
        ...
        <div class=views" style="padding-left: 50px; padding-top: 20px; width: 300px; height: 450px; margin-left: 1000px">
            <p style="color: LemonChiffon">Details</p>
            <label for="countryname" style="color: papayawhip">Selected Location</label>
            <input id="countryname" class="gamedisplay" placeholder="Selected Location" disabled> <br><br>
            <label for="countryname" style="color: papayawhip">Computer Density</label>
            <input id="numofcomps" class="gamedisplay" placeholder="Computer Density" disabled><br><br>
            <label for="countryname" style="color: papayawhip">Non-infected Computers</label>
            <input id="noninfec" class="gamedisplay" placeholder="Non-infected Computers" disabled><br><br>
            <label for="countryname" style="color: papayawhip">Infected Computers</label>
            <input id="infected" class="gamedisplay" placeholder="Infected Computers" disabled><br><br>
            <label for="countryname" style="color: papayawhip">Virus Protection</label>
            <input id="virusprotec" class="gamedisplay" placeholder="Virus Protection" disabled><br><br>
        </div>
    </div>
</div>
...
<script>
    ...
    function onload(){
        for(var x = 0; x <= 11; x++){
         ...
         document.getElementById('headline').placeholder = "Please select a location to start the virus, and ensure that the current date on the game is todays date."
    }
    ...
    function spread() {
        ...
        for (var x = 0; x <= 11; x++) {
			...
			if(fullyinfected[x] == "no"){
                    ...
                    fullyinfected[x] = "yes";
                    document.getElementById('headline').placeholder = countnames[x] + " has gone into darkness.";
			checkwin();
                }
		}
            } else {
...
var edate = 0;
var run = 0;
    function checkloss(day, month, year){
    ...
    function startarea(where){
        ...
        for(var x = 0; x <= 11; x++) {
            ...
            if (countnames[x] == where) {
                if(check == 0){
                    infectnumbs[x] = 1000;
                    document.getElementById('headline').placeholder = "The virus has been deployed in: " + where;
                    startclock();
                }
            }
        }
    ...
    function updateselected(){
        var find = document.getElementById('countryname').placeholder;
        for(var x = 0; x <= 11; x++){
            if(countnames[x] == find){
                document.getElementById('noninfec').placeholder = totalusers[x] - infectnumbs[x];
                document.getElementById('infected').placeholder = infectnumbs[x];
            }
        }
    }
	...
	checker = checker + 1;
        if(checker == 12){
            alert("well done, you successfully hacked the world, this means that the virus you created was succeful... Or you hacked the game to force yourself to win. Congradulations!");
        }
    }
</script>